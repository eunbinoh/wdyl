...
export default function WdylPromoSection() {
  const slides = useMemo(
    () => [
      ...
      {
        src: "/promo_img/pro_survey1.png",
        tag: "SURVEY",
        title: "심리테스트 같은 취향 설문지",
        desc: "티켓 링크를 받은 친구가 응답을 시작해요",
      },
      {
        src: "/promo_img/pro_survey2.png",
        tag: "STEP 1",
        title: "카테고리 선택",
        desc: "친구의 관심사를 10가지 선택지로 줄여줘요",
      },
      {
        src: "/promo_img/pro_survey3.png",
        tag: "STEP 2",
        title: "취향 토너먼트",
        desc: "1:1 라운드들을 통해 최종 후보 3개를 선정해요",
      },
      {
        src: "/promo_img/pro_survey4.png",
        tag: "PICK",
        title: "세부 항목 랭킹화",
        desc: "1가지 픽 아래 TOP3 순위를 알 수 있어요",
      },
      {
        src: "/promo_img/pro_result1.png",
        tag: "INSIGHT",
        title: "취향 분석 완료",
        desc: "추천템, 순위 결과에 특징을 반영해서 보여줘요",
      },
      {
        src: "/promo_img/pro_result2.png",
        tag: "RECOMMEND",
        title: "결과 확인 & 구매링크 추천",
  ...
  const TAG_BADGE_STYLE: Record<string, string> = {
    // 🟡 ticket
    TICKET: "bg-[#fff299] text-[#f4a800]",
    CONCEPT: "bg-[#fff299] text-[#f4a800]",
    TRENDY: "bg-[#fff299] text-[#f4a800]",
    // 🟢 survey
    SURVEY: "bg-[#c8f3d8] text-[#18a957]",
    "STEP 1": "bg-[#c8f3d8] text-[#18a957]",
    "STEP 2": "bg-[#c8f3d8] text-[#18a957]",
    PICK: "bg-[#c8f3d8] text-[#18a957]",
    // 🔵 result
    INSIGHT: "bg-[#d7e9ff] text-[#2b7fff]",
    RECOMMEND: "bg-[#d7e9ff] text-[#2b7fff]",
  };
  const getTagBadgeStyle = (tag: string) => TAG_BADGE_STYLE[tag] ?? "bg-gray-100 text-gray-600";
  const widthFitImageSources = useMemo(() => new Set(["/promo_img/pro_ticket1.png"]), []);
  ...
  return (
    <section className="mx-auto w-full max-w-[460px] px-5 py-10">
      <div className="mb-8 text-center">
        <p className="mb-2 text-sm font-bold tracking-[0.25em] text-[#f4ad24]">HOW TO USE</p>
        <p className="mt-4 text-base font-medium text-[#7b8496]">🟡 티켓 생성 → 🟢 설문링크 응답 → 🔵 선물 추천</p>
      </div>
        ...
        >
          {slides.map((slide) => (
            ...
            >
              <div>
                <span
                  className={`inline-flex rounded-full px-3 py-1 text-sm font-black ${getTagBadgeStyle(slide.tag)}`}
                >
                  {slide.tag}
                </span>
                <h3 className="mt-4 ml-2 text-xl font-black leading-tight text-[#222]">{slide.title}</h3>
                <p className="mt-2 ml-2 text-[14px] font-semibold leading-relaxed text-[#6f7890]">{slide.desc}</p>
              </div>
              ...
              <div className="flex justify-center">
                ...
                >
                  <img
                    ...
                    onLoad={(event) => {
                      const isLandscape = event.currentTarget.naturalWidth > event.currentTarget.naturalHeight;
