...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fetchError) {
      console.error("consume Payment 조회 실패:", fetchError);
    ...
    const update: { available_cnt: number; status?: string } = { available_cnt: nextAvailable };
    if (nextAvailable === 0) update.status = "use_done";

    const { error: updateError } = await supabase.from("Payment").update(update).eq("pay_id", payment.pay_id);

    if (updateError) {
      console.error("consume Payment 업데이트 실패:", updateError);
